...
**1. monitor.sh를 앱 실행 전에 먼저 완성한다**

이번에는 앱을 먼저 실행한 뒤 monitor.sh의 문제를 발견하여 여러 번 재실행했다. 다음에는 monitor.sh가 정상 동작하는 것을 확인한 후에 본격적인 장애 재현을 시작한다.

**2. 초기 MEMORY_LIMIT을 512MB로 설정하여 앱 동작을 충분히 관측한다**
...
**4. tee 명령어로 터미널 출력과 파일 저장을 동시에 한다**

```bash
/home/agent-leak-app-x86 2>&1 | tee /home/agent/agent-app/run.log
```

터미널에서 보면서 동시에 파일로도 저장되어 나중에 증거를 다시 찾을 필요가 없다.

**5. 각 케이스별 Before 데이터를 반드시 먼저 백업한 후 After로 넘어간다**
